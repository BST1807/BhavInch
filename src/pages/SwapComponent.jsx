...
const SwapComponent = () => {
    ...
    const fetchData = async () => {
      ...
      try {
        const balancesRes = await axios.get(
          "http://localhost:3001/api/balances",
          {
        ...
        const allTokensRes = await axios.get(
          "http://localhost:3001/api/all-tokens",
          {
            params: { chainId },
          }
        );
  ...
  useEffect(() => {
    const fetchQuote = async () => {
      ...
      const amountInWei = BigInt(
        Math.floor(amt * 10 ** fromToken.decimals)
      ).toString();
    ...
    const fetchGasPrice = async () => {
      ...
      try {
        const response = await axios.get(
          "http://localhost:3001/api/gas-price",
          {
            params: { chainId },
          }
        );
  ...
  const handleStellarBridge = async () => {
    ...
    try {
      ...
      const bridgeRes = await axios.post(
        "http://localhost:3001/api/stellar-bridge",
        {
      ...
      setBridgeStatus(
        `✅ Stellar bridge done: Issued ${toAmount} ${toToken.symbol} to Alice (Your account on Stellar Testnet)`
      );
  ...
  <span className="cursor-pointer flex items-center">
    ℹ️ <span className="ml-1 text-gray-500">Info</span>
  </span>
    ...
    It is recommended to use the native coin of the chain to avoid paying gas fees for approval.
          ...
          <>
            ...
            <div className="mt-6 p-4 bg-gradient-to-r from-gray-50 to-gray-100 rounded-xl text-sm text-gray-700 space-y-2">
              ...
              <div className="flex justify-between">
                ...
                <span className="text-gray-900">
                  {gasPrice
                    ? `${(Number(gasPrice.medium.maxFeePerGas) / 1e9).toFixed(
                        2
                      )} Gwei`
                    : "-"}
                </span>
        ...
        {swapTx && (
          <div className="mt-6 p-4 bg-gradient-to-r from-purple-50 to-purple-100 rounded-xl text-xs text-gray-700 space-y-2">
            ...
            <div>
              <span className="font-medium">From:</span> {swapTx.from}
            </div>
            <div>
              <span className="font-medium">To:</span> {swapTx.to}
            </div>
            <div>
              <span className="font-medium">Value:</span> {swapTx.value} WEI
            </div>
            <div>
              <span className="font-medium">Gas:</span> {swapTx.gas} GAS UNITS
            </div>
            <div>
              <span className="font-medium">GasPrice:</span> {swapTx.gasPrice}{" "}
              WEI
            </div>
            <div className="break-all">
              <span className="font-medium">Data:</span> {swapTx.data}
            </div>
          </div>
        )}
